...
Updated to route email tasks to the Email Agent.
"""
import os
...
Decision rules -- apply TOP TO BOTTOM, stop at first match:
  ...
  4.  code_result is not empty AND task does NOT mention github/save/commit/push           -> FINISH
  5.  code_result is not empty AND task mentions github/save/commit/push                   -> github
  6.  final_report is not empty AND task mentions code/implement/build/script/program      -> coder
  7.  final_report is not empty AND task mentions github/save/commit/push                  -> github
  8.  task mentions list/listing files OR create branch OR delete file (pure GitHub ops)   -> github
  9.  task mentions pdf/PDF/extract pdf/summarize pdf/read pdf/convert pdf/ocr             -> pdf
 10.  task mentions email/inbox/compose email/send email/reply/forward/mail/gmail/outlook  -> email
 11.  task mentions draft/subject line/phishing/unsubscribe/mail merge/drip campaign       -> email
 12.  task needs info (what/who/how/explain/research/why/latest/trends/history/compare)
      AND research_notes is empty                                                           -> research
 13.  research_notes is not empty AND final_report is empty                                -> writer
 14.  task mentions code/implement/build/script/program AND research_notes is empty        -> research
 15.  task mentions github/save/commit/push AND github_result is empty                     -> github
 16.  task is a greeting, small-talk, clarification, or simple question                    -> convo
 17.  convo_result is not empty                                                            -> FINISH
 18.  everything done                                                                      -> FINISH

CRITICAL:
  - Reply with EXACTLY ONE word. No punctuation. No explanation.
  - Valid replies: research | writer | coder | github | pdf | email | convo | FINISH
"""
...
def run_supervisor(state: AgentState) -> AgentState:
    ...
    if decision == "finish":
        decision = "FINISH"
    valid = {"research", "writer", "coder", "github", "pdf", "email", "convo", "FINISH"}
