/**
 * frontend/PDFAgent.jsx
 * PDF Agent UI — 8 features, calls the Python backend (/api/pdf)
 * which uses Groq + meta-llama/llama-4-scout-17b-16e-instruct
 * (same model stack as all other agents in this repo).
 *
 * Optimisations vs original:
 *  - No direct Anthropic calls — routes through Python pipeline
 *  - buildUserMessage is stable (no stale closure) via useRef
 *  - max_tokens bumped to 4096 to match backend
 *  - Ctrl/Cmd+Enter to run
 *  - sessionStorage history (survives tab refresh)
 *  - Clear history button
 *  - Accessible: aria-pressed, role=alert, role=status
 */
...
const FEATURES = [
  ...
  { id: "metadata",   icon: "ti-info-circle",      label: "Metadata",      desc: "Analyze or set PDF metadata" },
];
...
const PLACEHOLDERS = {
  ...
  merge_plan: "Describe the PDFs to merge and desired output (e.g. 'merge invoice.pdf + terms.pdf, invoice first, add page numbers')",
...
function CreateResult({ d }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {d.preview_text && <Card label="Content Preview"><p style={{ margin: 0, fontSize: 14, lineHeight: 1.7, color: "var(--color-text-secondary)" }}>{d.preview_text}</p></Card>}
      <div>
...
function ExtractResult({ d }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr>{tbl.headers?.map((h, j) => (
                  <th key={j} style={{ padding: "6px 10px", textAlign: "left",
                    borderBottom: "0.5px solid var(--color-border-tertiary)",
                    fontWeight: 500, color: "var(--color-text-secondary)", fontSize: 12 }}>{h}</th>
                ))}</tr>
              </thead>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 8 }}>
            {Object.entries(d.key_values).map(([k, v]) => (
              <div key={k} style={{ background: "var(--color-background-secondary)", borderRadius: 8,
      ...
      {d.entities && Object.entries(d.entities).filter(([, v]) => v?.length > 0).map(([type, items]) => (
        <div key={type}>
          <p style={{ fontSize: 12, fontWeight: 500, margin: "0 0 4px",
            color: "var(--color-text-secondary)", textTransform: "capitalize" }}>{type}</p>
...
function ReformatMergeResult({ d }) {
  const listItems = d.changes_made || d.recommendations || d.document_order || [];
      ...
      {listItems.length > 0 && (
        <div>
          ...
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {listItems.map((item, i) => <li key={i} style={{ fontSize: 14, marginBottom: 4 }}>{item}</li>)}
          </ul>
        </div>
      )}
      {d.python_code && <>
        <p style={{ fontSize: 13, color: "var(--color-text-secondary)", margin: "0 0 4px" }}>Python Code</p>
        <CodeBlock code={d.python_code} />
      </>}
    </div>
  );
...
function ResultRenderer({ feature, result }) {
  ...
  try {
    if (feature === "qa") return <p style={{ margin: 0, lineHeight: 1.7, fontSize: 14 }}>{result}</p>;
...
export default function PDFAgent() {
  ...
  const canRun   = !loading && (prompt.trim().length > 0 || pdfText.trim().length > 0);
  ...
  // Use a ref so run() never closes over stale prompt/pdfText/activeFeature
  const stateRef = useRef({ prompt, pdfText, activeFeature });
  useEffect(() => { stateRef.current = { prompt, pdfText, activeFeature }; }, [prompt, pdfText, activeFeature]);

  const run = useCallback(async () => {
    const { prompt: p, pdfText: pdf, activeFeature: feat } = stateRef.current;
    if (!p.trim() && !pdf.trim()) return;
    ...
    try {
      // Call the Python backend — same pipeline as all other agents
      // The backend runs: run_pdf_agent(state) with Groq
      const response = await fetch("/api/pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          task:     p || `${feat} the provided PDF content`,
          pdf_mode: feat,
          pdf_text: pdf,
        }),
      });
      ...
      const featObj = FEATURES.find(f => f.id === feat);
      setResult(clean);
      setHistory(h => [{
        feature:      feat,
        featureLabel: featObj?.label ?? feat,
        prompt:       p || "(PDF uploaded)",
        result:       clean,
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", maxWidth: 780, margin: "0 auto", padding: "1.5rem 1rem" }}>
      ...
      <h2 style={{ fontSize: 20, fontWeight: 500, margin: "0 0 4px" }}>
        <i className="ti ti-file-invoice" style={{ marginRight: 8, fontSize: 20, verticalAlign: -2 }} aria-hidden="true" />
        PDF Agent
      </h2>
      <p style={{ fontSize: 13, color: "var(--color-text-secondary)", margin: "0 0 20px" }}>
        AI-powered PDF toolkit — create, analyze, extract, and transform documents
        <span style={{ marginLeft: 6, fontSize: 11, opacity: 0.6 }}>· Groq · llama-4-scout</span>
      </p>

      {/* Feature grid */}
      <div role="group" aria-label="Select PDF feature"
        style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 8, marginBottom: 20 }}>
        {FEATURES.map(f => (
          <button key={f.id} onClick={() => handleFeatureChange(f.id)}
            ...
            <i className={`ti ${f.icon}`} style={{ fontSize: 18, display: "block", marginBottom: 4 }} aria-hidden="true" />
            <span style={{ fontSize: 12, fontWeight: 500, display: "block" }}>{f.label}</span>
            <span style={{ fontSize: 11, lineHeight: 1.4, display: "block",
              color: activeFeature === f.id ? "var(--color-text-info)" : "var(--color-text-secondary)" }}>{f.desc}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <i className={`ti ${feature.icon}`} style={{ fontSize: 16, color: "var(--color-text-secondary)" }} aria-hidden="true" />
          ...
          <div style={{ marginTop: 10 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
              <button onClick={() => fileRef.current.click()}
                ...
                <i className="ti ti-upload" style={{ fontSize: 14 }} aria-hidden="true" /> Upload PDF / text file
              </button>
              <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>or paste text below</span>
              <input ref={fileRef} type="file" accept=".txt,.pdf,.text"
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={run} disabled={!canRun}
            ...
            style={{ padding: "6px 16px", fontWeight: 500, opacity: !canRun ? 0.6 : 1, cursor: !canRun ? "not-allowed" : "pointer" }}>
            {loading
              ? <><i className="ti ti-loader-2" role="status" aria-label="Running"
                  style={{ marginRight: 6, fontSize: 14, animation: "spin 1s linear infinite" }} />Running...</>
              : <><i className="ti ti-player-play" style={{ marginRight: 6, fontSize: 14 }} aria-hidden="true" />Run Agent</>}
          </button>
          <span style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>⌘↵ to run</span>
          {(result || error) && (
            <button onClick={() => { setResult(null); setError(null); }}
              style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Clear</button>
          )}
        </div>
      </div>
      ...
      {result && (
        <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
            <i className="ti ti-check-circle" style={{ fontSize: 16, color: "var(--color-text-success)" }} aria-hidden="true" />
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <p style={{ fontSize: 13, fontWeight: 500, margin: 0, color: "var(--color-text-secondary)" }}>
              <i className="ti ti-history" style={{ marginRight: 4 }} aria-hidden="true" />Recent
            </p>
            <button onClick={() => setHistory([])}
              style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>Clear history</button>
          </div>
            ...
            {history.map((h, i) => (
              <div key={i} role="button" tabIndex={0}
                ...
                <span style={{ fontSize: 13, flex: 1, overflow: "hidden", textOverflow: "ellipsis",
                  whiteSpace: "nowrap", color: "var(--color-text-secondary)" }}>{h.prompt}</span>
